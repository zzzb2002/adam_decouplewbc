...
class PPO:
    ...
    def update(self):  # noqa: C901
        ...
        for sample in generator:
            ...
            data_augmentation_func: Callable[..., tuple[torch.Tensor | None, torch.Tensor | None]] | None = None
            if self.symmetry is not None:
                maybe_data_augmentation_func = self.symmetry["data_augmentation_func"]
                assert callable(maybe_data_augmentation_func)
                data_augmentation_func = cast(
                    Callable[..., tuple[torch.Tensor | None, torch.Tensor | None]],
                    maybe_data_augmentation_func,
                )
            ...
            if self.symmetry is not None and self.symmetry["use_data_augmentation"]:
                ...
                if next_obs_batch is not None:
                    next_obs_batch, _ = data_augmentation_func(
                        obs=next_obs_batch, actions=None, env=self.symmetry["_env"], obs_type="policy"
                    )
                    assert next_obs_batch is not None
                if next_critic_batch is not None:
                    next_critic_batch, _ = data_augmentation_func(
                        obs=next_critic_batch, actions=None, env=self.symmetry["_env"], obs_type="critic"
                    )
