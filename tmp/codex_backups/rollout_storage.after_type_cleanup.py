...
class RolloutStorage:
    ...
    def add_transitions(self, transition: Transition):
        ...
        if self.privileged_observations is not None:
            privileged_observations = transition.privileged_observations
            assert privileged_observations is not None
        ...
        if self.training_type == "distillation":
            ...
            self.privileged_actions[self.step].copy_(privileged_actions)
        ...
        if self.training_type == "rl":
            ...
            assert actions_log_prob is not None
            assert action_mean is not None
            assert action_sigma is not None
        ...
        if self.rnd_state_shape is not None:
            rnd_state = transition.rnd_state
            assert rnd_state is not None
            self.rnd_state[self.step].copy_(rnd_state)
    ...
    def _save_hidden_states(self, hidden_states):
        ...
        assert saved_hidden_states_a is not None
        assert saved_hidden_states_c is not None
        # copy the states
        for i in range(len(hid_a)):
            saved_hidden_states_a[i][self.step].copy_(hid_a[i])
            saved_hidden_states_c[i][self.step].copy_(hid_c[i])
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        next_observations_storage = torch.empty_like(self.observations)
        next_observations_storage[:-1] = self.observations[1:]
        next_observations_storage[-1] = self.observations[-1]
        if self.privileged_observations is not None:
            privileged_observations = self.privileged_observations.flatten(0, 1)
            next_critic_observations_storage = torch.empty_like(self.privileged_observations)
            next_critic_observations_storage[:-1] = self.privileged_observations[1:]
            next_critic_observations_storage[-1] = self.privileged_observations[-1]
        else:
            privileged_observations = observations
            next_critic_observations_storage = next_observations_storage
        next_observations = next_observations_storage.flatten(0, 1)
        next_critic_observations = next_critic_observations_storage.flatten(0, 1)
        ...
        not_dones_storage = torch.zeros_like(self.dones, dtype=torch.float)
        not_dones_storage[:-1] = 1.0 - self.dones[:-1].float()
        not_dones = not_dones_storage.flatten(0, 1)

        # For RND
        rnd_state = None
        if self.rnd_state_shape is not None:
            assert self.rnd_state is not None
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                yield obs_batch, privileged_observations_batch, next_obs_batch, next_critic_batch, cont_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (
                    None,
                    None,
        ...
        for ep in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                hid_a_batch = [
                    ...
                    .transpose(1, 0)
                    .contiguous()
                    for saved_hidden_states in saved_hidden_states_a
                ]
                hid_c_batch = [
                    saved_hidden_states.permute(2, 0, 1, 3)[last_was_done][first_traj:last_traj]
                    .transpose(1, 0)
                    .contiguous()
                    for saved_hidden_states in saved_hidden_states_c
                ]
