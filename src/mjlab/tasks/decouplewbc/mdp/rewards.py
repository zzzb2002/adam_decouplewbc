...
from .comamnd import UniformDecouplewbcCommandCfg
...
def track_base_height(
  ...
  command = env.command_manager.get_command(command_name)
  assert command is not None, f"Command '{command_name}' not found."
  height_error = torch.abs(command[:, 3] - asset.data.root_link_pos_w[:, 2])
...
def feet_slip(
  ...
  return cost
def foot_parallel_ground(
  env: ManagerBasedRlEnv,
  sensor_name: str,
  command_name: str | None = None,
  command_threshold: float = 0.05,
  min_air_time: float = 0.05,
  min_contact_steps: int = 3,
  asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
  """Encourage foot parallelism to ground when contact."""
  asset: Entity = env.scene[asset_cfg.name]
  contact_sensor: ContactSensor = env.scene[sensor_name]
  sensor_data = contact_sensor.data
  assert sensor_data.found is not None
  in_contact = (sensor_data.found > 0).float()
  foot_pos = asset.data.site_pos_w[:, asset_cfg.site_ids, :3]
  if foot_pos.shape[1] != 6:
    raise ValueError(
      f"feet_parallel expects 6 sites, got {foot_pos.shape[1]} from {asset_cfg.site_names}."
  )

  left_foot_height = foot_pos[:, :3, 2]
  right_foot_height = foot_pos[:, 3:, 2]
  left_foot_height_var = torch.var(left_foot_height, dim=-1, keepdim=True, unbiased=False)
  right_foot_height_var = torch.var(
    right_foot_height, dim=-1, keepdim=True, unbiased=False
  )
  feet_var = torch.cat([left_foot_height_var, right_foot_height_var], dim=1)
  ...
  landed_after_swing = (last_air_time >= min_air_time).float()
  stable_contact = (current_contact_time >= min_contact_steps * env.step_dt).float()
  active = landed_after_swing * stable_contact * in_contact

  cost = torch.sum(feet_var * active, dim=1)
  if command_name is not None:
    command = env.command_manager.get_command(command_name)
    if command is not None:
      linear_norm = torch.norm(command[:, :2], dim=1)
      angular_norm = torch.abs(command[:, 2])
      total_command = linear_norm + angular_norm
      command_active = (total_command > command_threshold).float()
      cost = cost * command_active
...
def feet_parallel(
  ...
  """Penalize non-parallel feet using three ordered outer-side sites per foot."""
  asset: Entity = env.scene[asset_cfg.name]
  command = env.command_manager.get_command(command_name)
  assert command is not None
  foot_pos = asset.data.site_pos_w[:, asset_cfg.site_ids, :3]
  ...
  active = (command[:, 3] >= height_threshold).float()
  return feet_distances_var * active


def knee_distance_lateral(
  env: ManagerBasedRlEnv,
  command_name: str,
  least_knee_distance_lateral: float,
  most_knee_distance_lateral: float,
  height_threshold: float,
  asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
  """Penalize hip/knee lateral distances outside a target range in the base frame."""
  asset: Entity = env.scene[asset_cfg.name]
  command = env.command_manager.get_command(command_name)
  assert command is not None
  body_pos_w = asset.data.body_link_pos_w[:, asset_cfg.body_ids, :3]
  if body_pos_w.shape[1] != 4:
    raise ValueError(
      f"knee_distance_lateral expects 4 hip/knee bodies, got {body_pos_w.shape[1]} "
      f"from {asset_cfg.body_names}."
    )

  body_pos_rel_w = body_pos_w - asset.data.root_link_pos_w.unsqueeze(1)
  body_pos_b = quat_apply_inverse(
    asset.data.root_link_quat_w.unsqueeze(1)
    .expand(-1, body_pos_rel_w.shape[1], -1)
    .reshape(-1, 4),
    body_pos_rel_w.reshape(-1, 3),
  ).reshape(env.num_envs, body_pos_rel_w.shape[1], 3)
  lateral_distance = (
    torch.abs(body_pos_b[:, 0, 1] - body_pos_b[:, 2, 1])
    + torch.abs(body_pos_b[:, 1, 1] - body_pos_b[:, 3, 1])
  )

  too_close = torch.clamp(
    lateral_distance - least_knee_distance_lateral * 2.0,
    max=0.0,
  )
  too_far = torch.clamp(
    most_knee_distance_lateral * 2.0 - lateral_distance,
    max=0.0,
  )
...
def deviation_knee_joint(
  ...
  """Penalize knee joint positions if they cross the soft limits."""
  asset: Entity = env.scene[asset_cfg.name]
  default_joint_pos_limits = asset.data.default_joint_pos_limits
  assert default_joint_pos_limits is not None
  command = env.command_manager.get_command(command_name)
  assert command is not None
  error = asset.data.root_link_pos_w[:, 2] - command[:,3]
  lower = default_joint_pos_limits[:, asset_cfg.joint_ids, 0]
  upper = default_joint_pos_limits[:, asset_cfg.joint_ids, 1]
  knee_deviation = (asset.data.joint_pos[:, asset_cfg.joint_ids] - lower) / (upper - lower)

  return torch.sum(torch.abs(knee_deviation-0.5)*error.unsqueeze(-1), dim=1)


def default_joint_position_l2_when_moving(
  env: ManagerBasedRlEnv,
  command_name: str,
  command_threshold: float,
  asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
  """Penalize selected joints moving away from default pose when commanded to move."""
  asset: Entity = env.scene[asset_cfg.name]
  default_joint_pos = asset.data.default_joint_pos
  assert default_joint_pos is not None
  command = env.command_manager.get_command(command_name)
  assert command is not None
  active = (command[:, 3] >= command_threshold).float()
  joint_pos = asset.data.joint_pos[:, asset_cfg.joint_ids]
  target_joint_pos = default_joint_pos[:, asset_cfg.joint_ids]
  error = joint_pos - target_joint_pos
  return torch.sum(torch.square(error), dim=1) * active
...
class variable_posture:
  ...
  def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv):
    ...
    command_term = env.command_manager.get_term(cfg.params["command_name"])
    assert command_term is not None
    command_cfg = cast(UniformDecouplewbcCommandCfg, command_term.cfg)
    self.default_base_height = command_cfg.default_base_height
  ...
  def __call__(
    ...
    height_active = (command[:, 3] >= (self.default_base_height-0.01)).float()

    return torch.exp(-torch.mean(error_squared / (std**2), dim=1)) * height_active
