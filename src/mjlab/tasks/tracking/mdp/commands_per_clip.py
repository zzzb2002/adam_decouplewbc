...
class MotionCommand(CommandTerm):
  ...
  def __init__(self, cfg: MotionCommandCfg, env: ManagerBasedRlEnv):
    ...
    if self._use_multi_clip:
        ...
        self._clip_bin_failed_counts = []  # 每个clip的失败计数
        self._clip_current_bin_failed = []  # 每个clip的当前失败计数
        self._clip_kernels = []  # 每个clip的kernel
        ...
        for i in range(self.motion.num_clips):
            ...
            self._clip_bin_failed_counts.append(
                torch.zeros(bin_count, dtype=torch.float, device=self.device)
            )
            self._clip_current_bin_failed.append(
                torch.zeros(bin_count, dtype=torch.float, device=self.device)
            )
            kernel = torch.tensor(
            ...
            self._clip_kernels.append(kernel / kernel.sum())
  ...
  def _adaptive_sampling(self, env_ids: torch.Tensor):
      ...
      # 1. 为每个 env 均匀随机选 clip
      clip_indices = torch.randint(
          0, self.motion.num_clips, (len(env_ids),), device=self.device
      )
      self._env_clips[env_ids] = clip_indices

      # 2. 按 clip 分组，每组内批量处理（clip 数量通常 10~50，env 4096）
      for clip_idx in range(self.motion.num_clips):
          mask = clip_indices == clip_idx
          clip_env_ids = env_ids[mask]
          if len(clip_env_ids) == 0:
              continue

          clip_start = boundaries_t[clip_idx]
          clip_end = boundaries_t[clip_idx + 1]
          clip_steps = clip_end - clip_start
          clip_bin_count = self._clip_bin_counts[clip_idx]

          # 更新失败统计（向量化）
          terminated = self._env.termination_manager.terminated[clip_env_ids]
          if torch.any(terminated):
              time_in_clip = self.time_steps[clip_env_ids] - clip_start
              current_bins = torch.clamp(
                  (time_in_clip * clip_bin_count) // max(clip_steps, 1),
                  0, clip_bin_count - 1,
              )
              fail_bins = current_bins[terminated]
              self._clip_current_bin_failed[clip_idx][:] = torch.bincount(
                  fail_bins, minlength=clip_bin_count
              ).float()

          # Adaptive sampling（向量化）
          bin_failed = self._clip_bin_failed_counts[clip_idx]
          sampling_prob = (
              bin_failed + self.cfg.adaptive_uniform_ratio / float(clip_bin_count)
          )
          ...
          sampling_prob = torch.nn.functional.conv1d(
              sampling_prob, self._clip_kernels[clip_idx].view(1, 1, -1)
          ).view(-1)
          sampling_prob = sampling_prob / sampling_prob.sum()

          sampled_bins = torch.multinomial(
              sampling_prob, len(clip_env_ids), replacement=True
          )
          self.time_steps[clip_env_ids] = clip_start + (
              (sampled_bins + sample_uniform(0.0, 1.0, (len(clip_env_ids),), device=self.device))
              / clip_bin_count * (clip_steps - 1)
          ).long()
          
          # Update metrics
          H = -(sampling_prob * (sampling_prob + 1e-12).log()).sum()
          H_norm = H / math.log(clip_bin_count) if clip_bin_count > 1 else 1.0
          pmax, imax = sampling_prob.max(dim=0)
          self.metrics["sampling_entropy"][clip_env_ids] = H_norm
          self.metrics["sampling_top1_prob"][clip_env_ids] = pmax
          self.metrics["sampling_top1_bin"][clip_env_ids] = imax.float() / clip_bin_count
  ...
  def _update_command(self):
    ...
    if self.cfg.sampling_mode == "adaptive":
        if self._use_multi_clip:
            for clip_idx in range(self.motion.num_clips):
                self._clip_bin_failed_counts[clip_idx] = (
                    self.cfg.adaptive_alpha * self._clip_current_bin_failed[clip_idx]
                    + (1 - self.cfg.adaptive_alpha) * self._clip_bin_failed_counts[clip_idx]
                )
                self._clip_current_bin_failed[clip_idx].zero_()
        else:
