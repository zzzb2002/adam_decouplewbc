...
# `python src/mjlab/scripts/train.py`, Python puts `src/mjlab/scripts`
...
def run_train(task_id: str, cfg: TrainConfig, log_dir: Path) -> None:
  ...
  if is_tracking_task:
    ...
    if motion_cmd.motion_dir and Path(motion_cmd.motion_dir).is_dir():
      print(f"[INFO] Loading all motions from directory: {motion_cmd.motion_dir}")
      npz_files = sorted(Path(motion_cmd.motion_dir).glob("*.npz"))
      for f in npz_files:
        print(f"  - {f.name}")
    elif motion_cmd.motion_files:
      print(f"[INFO] Loading {len(motion_cmd.motion_files)} motion files")
      for f in motion_cmd.motion_files:
        print(f"  - {Path(f).name}")
      ...
      raise ValueError(
        ...
        "  --env.commands.motion.motion-dir src/assets/motions/adam_sp\n"
